// BoundingObjects.cpp : Defines the entry point for the console application.
//
...
int main(int argc, const char** argv)
{
	...
	namedWindow(window_name_face, 1);
...
void detectFaceInFrame(Mat frame) {
	...
	for (size_t i = 0; i < faces.size(); i++)
	{
		...
		if (!image.empty()) {
			...
			cv::Rect roi(cv::Point(0, 0), cv::Size(50, 10));
			cv::Mat destinationROI = frame_gray(roi);
			image.copyTo(destinationROI(cv::Rect(0, 0, image.cols, image.rows)));
